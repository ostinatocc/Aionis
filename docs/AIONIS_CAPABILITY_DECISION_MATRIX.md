...
| 5 | Agent inspect/review/resume/handoff packs | `src/memory/agent-memory-inspect-core.ts` | Produces agent-facing recovery/review packets. | guide | Typed capability retained; internal HTTP adapters removed. | `product-core` keep | Converts memory into usable agent context. |
...
| 28 | Replay run lifecycle | `src/memory/replay*.ts` | Records replay/evidence runs and step outcomes. | observe, measure | Internal typed capability only; HTTP adapter removed. | `internal-evidence` keep | Replay is evidence engine, not product face. |
| 29 | Replay playbooks | `src/memory/replay*.ts`, `src/app/replay-*.ts` | Compiles/candidates/promotes/repairs/runs playbooks. | observe, guide, measure | Internal typed capability only; HTTP adapters removed. | `internal-evidence` keep | Keep for workflow learning; avoid repair-system positioning. |
...
| 61 | Raw memory find/resolve | `src/memory/find.ts`, `src/memory/resolve.ts`, `src/routes/memory-access.ts` | Low-level memory lookup; only resolve retains a stable HTTP support route. | support | Operator/internal only. | `review-support` hide | Useful for debugging, not product API. |
| 62 | Reviewer packs | `src/memory/reviewer-packs.ts` | Continuity/evolution review packs. | measure | Typed review/support capability; internal HTTP adapters removed. | `review-support` keep | Useful for audit, not default user path. |
