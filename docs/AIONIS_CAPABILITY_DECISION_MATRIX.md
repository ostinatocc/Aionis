...
| 11 | Planning/context assembly | `src/routes/memory-context-runtime.ts`, `src/app/planning-summary*.ts` | Builds planning summary and runtime context packet. | guide | Yes. Primary output. | `product-core` keep | This is how history reaches the Agent. |
...
| 15 | Kickoff recommendation | `src/memory/experience-intelligence.ts`, `src/app/planning-summary.ts` | Suggests first useful action. | guide | Visible as recommendation with uncertainty. | `product-core` keep | Directly reduces repeated discovery. |
...
| 30 | Controlled semantic forgetting | `src/kernel/forgetting-kernel.ts`, `src/memory/semantic-forgetting.ts` | Scores retain/demote/archive/review. | forget | Yes. Product capability. | `product-core` keep | One of the main product differentiators. |
| 31 | Archive relocation | `src/kernel/forgetting-kernel.ts`, `src/memory/archive-relocation.ts` | Moves cold memory while preserving payload refs. | forget | Visible as archive summary. | `product-core` keep | Better than blind deletion. |
| 32 | Rehydration | `src/memory/lifecycle-lite.ts`, `src/memory/rehydrate-anchor.ts`, `src/memory/differential-rehydration.ts` | Restores archived/payload memory when needed. | forget, guide | Visible as rehydration event. | `product-core` keep | Makes forgetting controllable. |
...
| 40 | Runtime boundary inventory | `src/server/lite-runtime-boundary.ts`, `src/routes/runtime-boundary-inventory.ts` | Reports product/kernel boundary and route matrix. | support | Visible support route. | `product-support` keep | Helps keep product boundary honest. |
...
|---|---|---|
| `observe` | memory write, execution state/handoff store, trajectory evidence, delegation records, execution evidence/provenance | benchmark-specific traces, external Agent host assumptions, source-code rules from a single run |
| `guide` | context assembly, execution packet, resume/handoff packet, action retrieval, experience intelligence, kickoff recommendation, authority labels | semantic patch generation, hard task constraints, direct replay repair as product promise |
