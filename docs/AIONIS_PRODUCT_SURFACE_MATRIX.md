...
HTTP POST /v1/guide -> agent_context.prompt_text
```
...
| Raw guide | `POST /v1/guide` | `guide()`, `execution.guideForRole()` | `agent_context.prompt_text` | `Stable` | `AionisRuntime-focused/src/routes/product-facade.ts:3469`, `aionis-sdk/src/index.ts:2066` | SDK guide tests, Runtime guide e2e |
...
| `@aionis/mcp` | MCP tool bridge | `aionis_context.agent_prompt` | `aionis-mcp/src/tools.ts:387`, `aionis-mcp/src/tools.ts:410` | `Integration` | Uses `guideForRole()` plus `compileExecutionAgentContext()`. Could later reuse `guideAgentContextForRole()`. |
| `@aionis/aifs` | File surface for file-reading Agents | `.aionis/guide.md` | `aionis-aifs/src/index.ts:565`, `aionis-aifs/src/index.ts:641` | `Integration` | Uses `guideForRole()` or `guide()` plus local compile. Lacks shared workspace scope derivation. |
| `aionis-claude-code` | Claude Code lifecycle hooks | injected context | `aionis-claude-code/packages/aionis-claude-code/src/index.ts:1116` | `Integration` | Uses `guideForRole()` plus local compile. Define priority with manual MCP refresh. |
...
| Build another final-Agent-context HTTP endpoint | `/v1/guide -> agent_context.prompt_text` is the HTTP contract; SDK adds top-level `agent_prompt`. |
...
2. Reduce integration duplication:

```text
MCP / AIFS / Claude Code should prefer execution.guideAgentContextForRole()
where practical, instead of repeating guideForRole() + compileExecutionAgentContext().
```
