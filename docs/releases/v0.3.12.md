# Aionis v0.3.12 Replay Upgrade Hardening Development

These notes describe an untagged Runtime-only development train for the
self-hosted Local Runtime Public Beta. v0.3.12 is not yet a published candidate:
there is no tag, official image, protected-provider run, publication receipt,
or GitHub prerelease.
...
|---|---:|---:|
| GitHub Runtime source | `v0.3.12` future tag | Not created; development metadata only. |
| Docker image | `ghcr.io/ostinatocc/aionis:v0.3.12` future candidate | No official digest exists. |
...
The development implementation and local tests are checkpoints, not release
evidence. Promotion requires all of the following against clean, exact commits:
...
2. A local `linux/amd64` image built from the clean v0.3.12 development commit.
