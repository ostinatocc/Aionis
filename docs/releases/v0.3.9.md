...
Before the immutable tag is pushed, the merged release commit still requires
its own CI result and the credential-injected `text-embedding-v4` exact-package
smoke. The tag-triggered workflow must then repeat frozen checkout, real tarball,
fresh-install, and digest-pinned Docker verification before the image or GitHub
