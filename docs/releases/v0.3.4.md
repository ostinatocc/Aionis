# Aionis v0.3.4 Release Candidate
...
composition, configuration, and dependency complexity. This document records
the candidate; the release is not final until the tag, image, and packages are
published.
...
- Unified Docker build and runtime stages on `node:24-bookworm-slim`, matching
  the validated candidate toolchain and removing the Substrate engine warning.
