...
| Controlled forgetting | `src/kernel/forgetting-kernel.ts`, `src/memory/lifecycle-lite.ts`, `src/memory/rehydrate-anchor.ts` | Demotes, archives, suppresses, or rehydrates memory instead of accumulating stale guidance forever. | Forgetting must be reversible or explainable when possible. |
| Judgment calibration | `src/memory/product-output-contract.ts`, `src/memory/product-output-assembler.ts`, `src/memory/operator-snapshot.ts` | Summarizes whether memory judgments were supported, contradicted, unused, weak, or inconclusive. | Read-only Phase 1; no ranking, authority, suppression, or lifecycle mutation. |
...
| `aionis_judgment_calibration_summary` | `src/memory/product-output-contract.ts`, `src/memory/product-output-assembler.ts`, `src/memory/operator-snapshot.ts` | Show read-only calibration of memory judgments across supported, contradicted, unused, weak, and inconclusive buckets. |
