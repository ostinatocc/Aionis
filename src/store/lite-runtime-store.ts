...
function parseJsonObject(raw: string | null | undefined): Record<string, unknown> {
  ...
  try {
    ...
    // Malformed stored JSON resolves to an empty object.
  }
  return {};
}
...
function createLiteRuntimeStoreSession(db: SqliteDatabase): LiteRuntimeStoreSession {
...
export function createLiteRuntimeStore(path: string): LiteRuntimeStore {
  ...
  return {
    backend: "lite_sqlite",
    async withClient<T>(fn: (session: LiteRuntimeStoreSession) => Promise<T>): Promise<T> {
      return fn(session);
    },
