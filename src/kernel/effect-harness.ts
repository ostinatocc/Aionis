...
export type AionisEffectHarnessInput = {
...
export type AionisEffectReport = {
  report_version: "aionis_effect_harness_v1";
  status: EffectStatus;
  baseline_score: number;
  aionis_score: number;
  effect_delta: number;
...
export function scoreAionisEffectObservation(observation: AionisEffectObservation): EffectKernelScore[] {
  ...
  if (expected.length !== present.length || expected.some((id) => !present.includes(id))) {
    throw new Error("effect harness must score every focused kernel capability");
  }
  return scores;
}
...
function compareKernelScores(args: {
  ...
  return args.aionisScores.map((aionisScore) => {
    ...
    if (!baselineScore) {
      throw new Error(`effect harness missing baseline score for ${aionisScore.capability_id}`);
    }
...
function nextActions(kernelScores: EffectKernelComparison[], status: EffectStatus): string[] {
  ...
  if (status === "pass") {
    actions.add("promote_effect_harness_to_real_run_fixtures");
  }
...
export function evaluateAionisEffect(args: AionisEffectHarnessInput): AionisEffectReport {
