...
export function createEvidencePromoteMemoryLearningControlReviewProvider(args?: {
  confidence?: number;
  reason?: string;
}): PromoteMemoryLearningControlReviewProvider {
  const confidence = args?.confidence ?? 0.84;
  const reason = args?.reason ?? "evidence provider found workflow-signature evidence";
  return {
...
export function createEvidenceFormPatternLearningControlReviewProvider(args?: {
  confidence?: number;
  reason?: string;
}): FormPatternLearningControlReviewProvider {
  const confidence = args?.confidence ?? 0.85;
  const reason = args?.reason ?? "evidence provider found grouped signature evidence";
  return {
    resolveReviewResult: ({ reviewPacket, suppliedReviewResult }) => {
      if (suppliedReviewResult) {
        return suppliedReviewResult;
