...
} from "../../src/kernel/effect-evaluator.ts";
import { aionisKernelCapabilityIds } from "../../src/kernel/boundary.ts";

test("effect evaluator scores every focused kernel capability", () => {
...
test("effect evaluator proves Aionis improves execution behavior over a baseline run", () => {
  ...
  assert.equal(report.report_version, "aionis_effect_evaluator_v1");
...
test("effect evaluator warns when the run is safe but not measurably better", () => {
  const safeRun = {
    continuity: {
      repeatedDiscoverySteps: 0,
      continuityGuidanceCorrect: true,
