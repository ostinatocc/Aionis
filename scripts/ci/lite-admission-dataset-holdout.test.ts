...
test("admission dataset holdout splits real Runtime rows by task signature", () => {
  ...
  assert.equal(report.split.group_count, 7);
  assert.equal(report.split.train_group_count + report.split.holdout_group_count, 7);
  assert.equal(report.split.train_row_count + report.split.holdout_row_count, 105);
...
test("admission dataset holdout can split by run id for chunk-like validation", () => {
  ...
  assert.equal(report.split.group_count, 49);
  assert.equal(report.split.train_row_count + report.split.holdout_row_count, 105);
  assert.equal(report.checks.disjoint_groups, true);
