...
function standaloneSource(runtimeSource) {
  return runtimeSource.replace(
    /^\/\/ Runtime reference SDK implementation\.\r?\n\/\/ This file is the canonical source for the standalone SDK\.\r?\n/,
    "// Standalone @aionis/sdk client implementation.\n// Runtime is the canonical source for this client.\n",
  );
}
...
const expected = standaloneSource(source);

if (command === "sync") {
  if (expected !== target) {
    fs.mkdirSync(path.dirname(targetFile), { recursive: true });
    fs.writeFileSync(targetFile, expected);
    console.log(`Synced canonical Runtime SDK source into ${targetFile}`);
  } else {
    console.log(`Canonical SDK source already in sync: ${targetFile}`);
  }
  process.exit(0);
}

if (expected !== target) {
  console.error("Standalone @aionis/sdk source is out of sync with Runtime.");
